...
extension UITableView {
    ...
    func dx_heightForCell(identifier: String, configuration: ((cell: UITableViewCell) -> Void)?) -> CGFloat {
        ...
        let templatecCell = self.dx_templateCell(identifier)
        if let cell = templatecCell {
            ...
            if (autoLayoutEnabled) {
                ...
                let tempWidthConstraint = NSLayoutConstraint(item: cell.contentView, attribute: NSLayoutAttribute.Width, relatedBy: NSLayoutRelation.Equal, toItem: nil, attribute: NSLayoutAttribute.NotAnAttribute, multiplier: 1, constant: contentViewWidth)
    ...
    func dx_heightForCell(identifier: String, cacheByIndexPath: NSIndexPath, configuration: ((cell: UITableViewCell) -> Void)?) -> CGFloat {
        ...
        if (self.dx_cellHeightCache.hasCachedHeightAtIndexPath(cacheByIndexPath)) {
            let height = self.dx_cellHeightCache.cachedHeightAtIndexPath(cacheByIndexPath)
        ...
        let height = self.dx_heightForCell(identifier, configuration: configuration)
    ...
    var dx_debuglogEnabled: Bool? {
        ...
        set {
            if let value = newValue {
                objc_setAssociatedObject(self, &AssociatedKey.dx_debuglogEnabled_key, NSNumber(bool: value), UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            } else {
                objc_setAssociatedObject(self, &AssociatedKey.dx_debuglogEnabled_key, nil, UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            }
        }
    }
    ...
    private var dx_templateCellsByIdentifiers: NSMutableDictionary? {
        ...
        set {
            objc_setAssociatedObject(self, &AssociatedKey.dx_templateCellsByIdentifiers_key, newValue, UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC))
        }
    }
    ...
    private var dx_autoCacheInvalidationEnabled: Bool? {
        ...
        set {
            if let value = newValue {
                objc_setAssociatedObject(self, &AssociatedKey.dx_autoCacheInvalidationEnabled_key, NSNumber(bool: value), UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            } else {
                objc_setAssociatedObject(self, &AssociatedKey.dx_autoCacheInvalidationEnabled_key, nil, UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            }
        }
    }
    ...
    private var dx_precacheEnabled: Bool? {
        ...
        set {
            if let value = newValue {
                objc_setAssociatedObject(self, &AssociatedKey.dx_precacheEnabled_key, NSNumber(bool: value), UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            } else {
                objc_setAssociatedObject(self, &AssociatedKey.dx_precacheEnabled_key, nil, UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            }
        }
    }
    ...
    private func dx_allIndexPathsToBePrecached() -> [NSIndexPath] {
        ...
        for section in 0..<numberOfSections {
            let numberOfRowsInSection = self.numberOfRowsInSection(section)
    ...
    private var dx_cellHeightCache: DXTemplateLayoutCellHeightCache {
        get {
            ...
            if (nil == cache) {
                ...
                objc_setAssociatedObject(self, &AssociatedKey.dx_cellHeightCache_key, cache, UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC));
            }
...
extension UITableViewCell {
    ...
    var dx_isTemplateLayoutCell: Bool? {
        ...
        set {
            if let value = newValue {
                objc_setAssociatedObject(self, &AssociatedKey.dx_isTemplateLayoutCellKey, NSNumber(bool: value), UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            }
        }
    }
    ...
    var dx_enforceFrameLayout: Bool? {
        ...
        set {
            if let value = newValue {
                objc_setAssociatedObject(self, &AssociatedKey.dx_enforceFrameLayout, NSNumber(bool: value), UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_COPY))
            }
        }
    }
}
...
//    }
    ...
    func hasCachedHeightAtIndexPath(indexPath: NSIndexPath) -> Bool {
        let cachedHeight = self.cachedHeightAtIndexPath(indexPath)
    ...
    func cachedHeightAtIndexPath(indexPath: NSIndexPath) -> CGFloat? {
        let sectionCache = self.sections.objectForKey(indexPath.section) as? NSMapTable
        let cachedHeight = sectionCache?.objectForKey(indexPath.row) as? CGFloat
...
extension UITableView {
    ...
    func dx_moveSection(section: NSInteger, toSection: NSInteger) -> Void {
        if let notNilAutoCache = self.dx_autoCacheInvalidationEnabled {
            if (notNilAutoCache) {
                let sectionCache1 = self.dx_cellHeightCache.sections.objectForKey(section) as? NSMapTable
                let sectionCache2 = self.dx_cellHeightCache.sections.objectForKey(toSection) as? NSMapTable
        ...
        if let notNilAutoCache = self.dx_autoCacheInvalidationEnabled {
            if (notNilAutoCache) {
                indexPaths.enumerateObjectsUsingBlock({ (indexPath, index, stop) -> Void in
                    let sectionCache = self.dx_cellHeightCache.sections.objectForKey(indexPath.section) as? NSMapTable
    ...
    func dx_moveRowAtIndexPath(sourceIndexPath: NSIndexPath, toIndexPath: NSIndexPath) -> Void {
        if let notNilAutoCache = self.dx_autoCacheInvalidationEnabled {
            if (notNilAutoCache) {
                let sectionCache1 = self.dx_cellHeightCache.sections.objectForKey(sourceIndexPath.section) as? NSMapTable
                let sectionCache2 = self.dx_cellHeightCache.sections.objectForKey(toIndexPath.section) as? NSMapTable
                
                let rowCache1 = sectionCache1?.objectForKey(sourceIndexPath.row) as? CGFloat
                let rowCache2 = sectionCache2?.objectForKey(toIndexPath.row) as? CGFloat
