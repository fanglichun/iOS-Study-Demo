...
class ViewController: UITableViewController, UIActionSheetDelegate {
    ...
    func buildTestDataThen(then: () -> Void) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), { () -> Void in
            ...
            let data = NSData.dataWithContentsOfMappedFile(dataFilePath!) as! NSData
            let rootDic = (try? NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.AllowFragments)) as? NSDictionary
            
            let entities = NSMutableArray()
            let feedDics = rootDic?.objectForKey("feed") as? NSArray
            feedDics?.enumerateObjectsUsingBlock({ (obj, index, stop) -> Void in
                let dic = obj as! NSDictionary
                let entity = DXFeedEntity(dictionary: dic)
    ...
    override func numberOfSectionsInTableView(tableView: UITableView) -> Int {
        let count = self.feedEntitySections?.count
    ...
    override func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        let value = self.feedEntitySections?.objectAtIndex(section) as! NSArray
        return value.count
    }
    
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell: DXFeedCell = tableView.dequeueReusableCellWithIdentifier("DXFeedCell", forIndexPath: indexPath) as! DXFeedCell
    ...
    func configureCell(cell: DXFeedCell, atIndexPath: NSIndexPath) -> Void {
        ...
        let value = self.feedEntitySections?.objectAtIndex(atIndexPath.section).objectAtIndex(atIndexPath.row) as? DXFeedEntity
        cell.entity = value
    }
    ...
    override func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
        ...
        if (self.cellHeightCacheEnabled!) {
            value = tableView.dx_heightForCell("DXFeedCell", cacheByIndexPath: indexPath) { (cell) -> Void in
                let dxCell: DXFeedCell = cell as! DXFeedCell
    ...
    @IBAction func rightNavigationItemAction() {
        let actionSheet = UIActionSheet(title: "Actions", delegate: self, cancelButtonTitle: "Cancel", destructiveButtonTitle: nil, otherButtonTitles: "Insert a row", "Insert a section", "Delete a section")
    ...
    func actionSheet(actionSheet: UIActionSheet, clickedButtonAtIndex buttonIndex: Int) {
        ...
        if (index < selectors.count) {
            let sel: Selector = Selector(selectors[index])
    ...
    func randomEntity() -> DXFeedEntity? {
        ...
        if let jsonArray = self.prototypeEntitiesFromJSON {
            ...
            let randomEntity = jsonArray[Int(randomNumber)] as! DXFeedEntity
    ...
    func insertRow() {
        ...
        let entity = self.randomEntity()
        if let notNilEntity = entity {
            self.tableView.beginUpdates()
            let arr = self.feedEntitySections?.objectAtIndex(0) as! NSMutableArray
